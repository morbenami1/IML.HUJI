...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        for i in range(self.iterations_):
            ...
            epsilon = np.sum(self.D_[np.sign(model_pred) != np.sign(y)])
            self.weights_[i] = 0.5 * np.log((1 - epsilon) / epsilon)
            self.D_ *= np.exp(-y * self.weights_[i] * model_pred)
    ...
    def partial_loss(self, X: np.ndarray, y: np.ndarray, T: int) -> float:
        ...
        loss : float
            Performance under missclassification loss function
        """
        return loss_functions.misclassification_error(self.partial_predict(X, T), y)
